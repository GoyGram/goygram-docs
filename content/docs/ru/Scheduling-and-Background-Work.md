...
[State entries](/ru/docs/Keyboards-Formatting-and-State) can receive a TTL. GoyGram's runtime starts the FSM cleanup loop automatically, so expired entries disappear while `app.run()` is active.
