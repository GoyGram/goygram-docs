---
title: "Обработка запроса обратного вызова"
...
Параметр `via=self.src` гарантирует, что редактирование проходит через тот же транспорт, который доставил обратный вызов (для обратных вызовов используется только Bot API, поскольку в MTProto нет встроенных клавиатур).
...
`CbObj` is inherently Bot API-only. Поле `src`, хотя и присутствует, всегда имеет значение `"bot"`, поскольку запросы обратного вызова недоступны в необработанном MTProto. И `answer()`, и `edit()` внутренне полагаются на `app.bot` (BotNet).
