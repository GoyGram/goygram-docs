...
Основная идея: `update_hook` всегда срабатывает после типизированных обработчиков. `StopPropagation` из обработчика `on_msg` только останавливает цепочку обработчиков сообщений — всеобработчики `on_update` по-прежнему выполняются.
