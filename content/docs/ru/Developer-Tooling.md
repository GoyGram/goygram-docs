...
Встроенные инструменты самоанализа для изучения поверхности API GoyGram.

## `app.help()` / `print_methods(app)`

Выводит обзор разработчика на консоль:
...
Показывает все доступные методы, включая динамические записи:
...
Дополнение `__dir__` добавляет общие динамические методы:
...
## Мониторинг шины событий

Внутренний `asyncio.Queue` (`app.core.bus.q`) переносит все события до того, как они достигнут обработчиков. Вы можете использовать его для отладки:
...
## Профилирование обработчика

Оберните обработчики для измерения времени выполнения:
...
Проверьте низкоуровневое соединение MTProto:
...
Объекты событий GoyGram используют `__slots__` с аннотациями типов. Запустите mypy, чтобы отловить ошибки обработчика:
...
## Проверка обработчика уровня AST
