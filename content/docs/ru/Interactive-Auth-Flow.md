...
Когда `bootstrap_session()` не может найти действительный файл `.vault` и не существует файла `.session` для миграции, GoyGram запускает интерактивный процесс авторизации на терминале. Это полный опыт входа в систему — ввод номера телефона, проверка кода, дополнительный 2FA и сканирование QR-кода.
