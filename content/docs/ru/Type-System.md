...
Для динамической сериализации TL во время выполнения расширение `serialize_method()` в Rust обрабатывает любой метод, определенный в загруженной схеме `.tl` — для стандартных вызовов MTProto не требуются определения типов на стороне Python.