...
- **Основная архитектура** — Как на самом деле работает разделенная архитектура Python+Rust. Топология шины событий, конвейер диспетчера и система динамического разрешения методов, которая позволяет вызывать `app.sendDocument(...)`, не являясь жестко закодированным методом.
...
- **Полный справочник по клиентскому API** — каждый общедоступный метод, каждый тип событий (`MsgObj`, `CbObj`, `PollObj`, `MemberObj`), система фильтров с логической композицией, маршрутизация команд и построители клавиатуры.
...
- **Расширенные шаблоны** – многосессионное фермерство, двойная транспортная маршрутизация, туннелирование прокси-сервера (SOCKS5 + HTTP CONNECT), динамическая миграция контроллеров домена при ошибках `PHONE_MIGRATE` и жизненный цикл токена входа в систему с помощью QR-кода.
