---
title: "Дизайн с разделенным мозгом"
...
Потому что каждый транспорт имеет свой собственный жизненный цикл соединения, свой цикл обновления, свой формат сериализации и собственную обработку ошибок, но они сходятся в одном и том же `asyncio.Queue` и одной и той же диспетчеризации обработчика. Правая половина (Bot API/HTTP) не знает, что левая половина (MTProto/TCP) существует. Это два мозга в одном теле.
...
| Конфликт вебхука | Автоматически очищается на 409 | Н/Д |
