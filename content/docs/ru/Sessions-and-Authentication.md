...
## Кэш сущностей в vault

MTProto-транспорт держит в памяти кэш сущностей: пары `id + access_hash` и `username` для пользователей, чатов и каналов. Кэш заполняется автоматически из каждого ответа сервера (`users` / `chats` в ответах TL) и используется при разрешении пиров, поэтому холодный запуск больше не вызывает лишних `messages.getDialogs`.

Кэш персистится прямо в vault сессии:

- запись — с задержкой 30 секунд после изменения (debounce), а при завершении работы (`await app.close()`) — принудительно;
- формат — минимальные поля `id`, `access_hash`, `username` (без фото, имён и прочего «жирного» мусора);
- лимиты — 3000 пользователей и 1500 чатов/каналов отдельно, плюс общий потолок 512 КБ на весь снимок;
- вытеснение — по взвешенному LRU: давно не использованные сущности уходят первыми, `self` и явно взвешенные сущности (`mt._entity_weights[(kind, id)] = 10.0`) держатся дольше всех;
- чтение — при старте, до первого сетевого запроса, поэтому бот-сессии после рестарта сразу знают `access_hash` каналов и больше не падают с «channel peer requires a non-zero access_hash».

Аналогично в vault кэшируются `dc_auth_keys` — авторизационные ключи файловых датацентров, используемые параллельными загрузками (см. [Файлы и медиа](/docs/Files-and-Media)).
