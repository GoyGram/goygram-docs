...
Если HTTP-соединение разрывается, `aiohttp` прозрачно создает новое TCP-соединение при следующем запросе. Объект сеанса обрабатывает пул соединений.
...
2. Все ожидающие фьючерсы не состоялись, за исключением
