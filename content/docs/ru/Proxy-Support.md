...
Все ошибки прокси вызывают `ConnectionError` с описательными сообщениями. Это распространяется через `MTNet.boot()` и в конечном итоге приводит к сбою `MTNet.spin()`, что приводит к остановке задачи MTProto. Приложение не повторяет автоматически повторную попытку с другим прокси-сервером — если прокси-сервер выходит из строя, соединение прерывается.