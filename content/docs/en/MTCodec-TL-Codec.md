...
`MTCodec` is a **flat class** — no inheritance, no abstraction layers. Each MTProto method has a dedicated method that constructs the TL bytes. Constructor IDs are class-level constants.

## Constructor ID Registry (Partial)
...
## initConnection Wrapping

The first message after DH key exchange must be wrapped with `invokeWithLayer` and `initConnection`:
...
Each MTProto action has a dedicated builder method. Example — send message:
...
Used for parsing server responses and RPC results.
